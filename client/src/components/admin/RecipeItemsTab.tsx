...
const RecipeItemsTab: React.FC = () => {
  const { menus, currentMenu, ingredientList, isLoading, error } = useMenuData();
  ...
  const itemsWithCost = recipeItems.map(item => ({
    ...item,
    cost: calculateRecipeCost(item.ingredients || {}, ingredientList, currentMenu.costMultiplier)
  }));

  let filteredItems = itemsWithCost;
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-navy tracking-wide">Recipe Items</h2>
        <button 
      ...
      {isLoading ? (
        ...
        <p className="text-center text-gray-500">No recipe items found.</p>
      ) : (
                ...
                <tbody>
                  {itemsByCategory[category].map(item => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <td className="text-right text-gray-800 px-4 py-3 border-b border-gray-200">£{item.cost.toFixed(2)}</td>
